...
//okno
	...
	void okno::draw(sf::RenderTarget& target, sf::RenderStates states) const
	{
		...
		if(wyswietlaj_tytul)target.draw(tytul, states);

	}

	okno::okno()
	{
		wyswietlaj_tytul == true;
	...
	okno::okno(Vector2f rozmiar,string nazwa, Color c_ramka, Color c_tlo)
	{
		wyswietlaj_tytul == true;
		...
		tytul.setPosition(2, 1);
		kolizja_tlo = tlo.getGlobalBounds();
		kolizja_ramka = ramka.getGlobalBounds();
	}


	void okno::SetSize(Vector2f rozmiar)
	{
		
		ramka.setSize(rozmiar);
		tlo.setSize(Vector2f(rozmiar.x - 4, rozmiar.y - 25));
		zamknij.setSize(Vector2f(20, 20));
		if (rozmiar.x < tytul.getGlobalBounds().width)
		{
			wyswietlaj_tytul == false;
		}
		zamknij.setPosition(rozmiar.x - 20, 0);
		kolizja_tlo = tlo.getGlobalBounds();
		kolizja_ramka = ramka.getGlobalBounds();
